...
/**
 * @ngdoc function
 * @name homeWorkApp.controller:newsUserQueryController
 * @description
 * # newsUserQueryController
 * Controller of the homeWorkApp
 */
 var app = angular.module('homeWorkApp');


 app.controller('newsUserQueryController', ['$scope','$filter', 'newsUserQueryService', function ($scope,
    $filter,newsUserQueryService) {
 
   
    var categoryReturn = newsUserQueryService.getCategory();
...
$scope.addDate = function() { 	
    ...
    $scope.releasedNewsByFlexibleSearch = function () {
       ...
       paramNewsFlexibleDto["createDateTo"] =dateToStr($scope.date.endDate);
        ...
        var releasedNewsByFlexibleSearchReturn  = newsUserQueryService.releasedNewsByFlexibleSearch("5",$scope.pageableDto.pageNumber,paramNewsFlexibleDto);
